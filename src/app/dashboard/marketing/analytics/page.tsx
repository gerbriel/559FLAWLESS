...
export default async function AnalyticsPage({ searchParams }: Props) {
  ...
  // The section layout gates this too, but a layout is not where an auth check
  // can live alone: layouts do not re-render on a client-side transition
  // (partial rendering), so a tab click from a sibling page never re-runs it.
  // `manager reads analytics` (migration 009) is the real boundary; this is the
  // same check the layout makes, said again where the data is actually read.
  const {
    data: { user },
  } = await supabase.auth.getUser()
  if (!user) redirect('/login?next=/dashboard/marketing/analytics')
      ...
      <div className="mt-8 flex flex-wrap gap-x-8 gap-y-4">
        <nav className="flex gap-5" aria-label="Date range">
          {RANGES.map((r) => (
            <Link
              key={r}
              href={`/dashboard/marketing/analytics?days=${r}&segment=${segment}`}
        ...
        <nav className="flex gap-5" aria-label="Segment">
          {SEGMENTS.map((s) => (
            <Link
              key={s.key}
              href={`/dashboard/marketing/analytics?days=${days}&segment=${s.key}`}
              className={`label-caps pb-1 ${
                segment === s.key
